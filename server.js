...
db.exec(`CREATE TABLE tags (id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT UNIQUE NOT NULL)`);
db.exec(`CREATE TABLE note_tags (note_id INTEGER,tag_id INTEGER,PRIMARY KEY (note_id, tag_id),FOREIGN KEY (note_id) REFERENCES notes (id) ON DELETE CASCADE,FOREIGN KEY (tag_id) REFERENCES tags (id) ON DELETE CASCADE)`);
db.exec(`CREATE TABLE collaborations (id INTEGER PRIMARY KEY AUTOINCREMENT,admin_id INTEGER NOT NULL,user_id INTEGER NOT NULL,created_at DATETIME DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY (admin_id) REFERENCES users (id),FOREIGN KEY (user_id) REFERENCES users (id))`);
db.exec(`CREATE TABLE note_versions (id INTEGER PRIMARY KEY AUTOINCREMENT,note_id INTEGER NOT NULL,title TEXT NOT NULL,body TEXT NOT NULL,version_number INTEGER NOT NULL,created_at DATETIME DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY (note_id) REFERENCES notes (id) ON DELETE CASCADE)`);
...
app.get('/notes', auth, (req, res) => {
    const { tag } = req.query;
    let query = `SELECT n.*, u.username as author, GROUP_CONCAT(t.name) as tagsFROM notes nJOIN users u ON n.author_id = u.idLEFT JOIN note_tags nt ON n.id = nt.note_idLEFT JOIN tags t ON nt.tag_id = t.id`;
    let params = [];
    if (tag) {
        query += ` WHERE n.id IN (SELECT DISTINCT n.id FROM notes nJOIN note_tags nt ON n.id = nt.note_idJOIN tags t ON nt.tag_id = t.idWHERE t.name = ?)`;
        params.push(tag);
    }
    query += ' GROUP BY n.id ORDER BY n.created_at DESC';
    const stmt = db.prepare(query);
    const rows = stmt.all(...params);
    const notes = rows.map(row => ({...row,tags: row.tags ? row.tags.split(',') : []}));
    res.json(notes);
});
...
app.post('/invite', auth, (req, res) => {
    ...
    const user = getUser.get(email);
    if (!user) return res.status(404).json({ error: 'User lost ' });
    ...
    res.json({ message: 'Collaborator invited to notes' });
});
